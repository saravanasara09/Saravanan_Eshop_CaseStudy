...
const ListProducts = ({
    ...
    if (s !== "DEFAULT") {
      list.sort((a, b) => {
        if (s === "PRICE_ASC") {
          ...
          //default
          return 0; //Return 0 fro default and other cases
        }
      });
    }
    return list;
  ...
  let getFilteredProducts = (list, c) => {
    ...
    for (let i = 0; i < list.length; i++) {
      if (
        c.toUpperCase() === "ALL" ||
        c.toUpperCase() === list[i].category.toUpperCase()
      ) {
        filteredList.push(list[i]);
      }
    }
  ...
  let products = getFilteredProductsBasedOnQuery(
